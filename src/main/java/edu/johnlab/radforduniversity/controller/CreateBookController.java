...
public class CreateBookController implements Initializable {
    ...
    public CreateBookController(SceneManager sceneManager, UserAuth userModel, CategoryRepository categoryRepository, BookRepository bookRepository) {
    ...
    public void handleSaveBook() {
        if (cmbCategoria.getValue() == null || txtFieldTitle.getText().isBlank() || txtFieldAuthor.getText().isBlank() || txtFieldPublisher.getText().isBlank()
            ...
            try {
                ...
                System.out.println("DEBUG: Insertando Libro -> ISBN: " + idUnico + " | idCategory: " + idCategory + " | idUser: " + idUser);

                Book libroNuevo = new Book(idUnico.toString(), idCategory, idUser, title, author, publisher, publicationYear, availableCopies);

                boolean guardado = bookRepository.create(libroNuevo);

                if (guardado) {
                    sceneManager.showInfoAlert("Guardado", "Libro creado", "El libro se guardó correctamente", Alert.AlertType.INFORMATION);
